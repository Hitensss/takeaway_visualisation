...
object DeliveryFeeTimeAnalysis {
  ...
  def analyze(spark: SparkSession, cleanedDF: DataFrame): Unit = {
    ...
    val highAvgTime = highFeeDF.select(avg("delivery_time")).collect()(0).getDouble(0)
    val freeAvgTime = freeFeeDF.select(avg("delivery_time")).collect()(0).getDouble(0)
    val highMedianTime = highFeeDF
      .selectExpr("cast(percentile_approx(delivery_time, 0.5) as double)")
      .collect()(0)
      .getDouble(0)
