...
const loadData = async () => {
  try {
    const res = await deliveryAPI.getFeeTimeScatter()
    if (res.data.code === 200) {
      renderChart(res.data.data)
    }
  } catch (error) {
    console.error('加载配送费-送达时间散点图失败:', error)
  }
}

const renderChart = (data) => {
  chart = echarts.init(chartRef.value)
  const scatterData = data.map((d) => [d.deliveryFee, d.deliveryTime])
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `配送费: ¥${params[0].value[0]}<br/>送达时间: ${params[0].value[1]}分钟`
      },
    },
    xAxis: {
      name: '配送费（元）',
