...
const loadData = async () => {
  try {
    const res = await merchantAPI.getSalesDistanceScatter()
    if (res.data.code === 200) {
      renderChart(res.data.data)
    }
  } catch (error) {
    console.error('加载月售-距离散点图失败:', error)
  }
}

const renderChart = (data) => {
  chart = echarts.init(chartRef.value)
  const scatterData = data.map((d) => [d.distance, d.monthlySales])
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `距离: ${params[0].value[0]}米<br/>月售: ${params[0].value[1]}单`
      },
    },
    xAxis: {
      name: '距离（米）',
