...
const loadData = async () => {
  try {
    const res = await merchantAPI.getMinPriceRatingScatter()
    if (res.data.code === 200) {
      renderChart(res.data.data)
    }
  } catch (error) {
    console.error('加载起送价-评分散点图失败:', error)
  }
}

const renderChart = (data) => {
  chart = echarts.init(chartRef.value)
  const scatterData = data.map((d) => [d.minPrice, d.rating])
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `起送价: ¥${params[0].value[0]}<br/>评分: ${params[0].value[1]}分`
      },
    },
    xAxis: {
      name: '起送价（元）',
