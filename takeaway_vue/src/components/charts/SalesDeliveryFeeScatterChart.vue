...
const loadData = async () => {
  try {
    const res = await merchantAPI.getSalesDeliveryFeeScatter()
    if (res.data.code === 200) {
      renderChart(res.data.data)
    }
  } catch (error) {
    console.error('加载月售-配送费散点图失败:', error)
  }
}

const renderChart = (data) => {
  chart = echarts.init(chartRef.value)
  const scatterData = data.map((d) => [d.deliveryFee, d.monthlySales])
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `配送费: ¥${params[0].value[0]}<br/>月售: ${params[0].value[1]}单`
      },
    },
    xAxis: {
      name: '配送费（元）',
