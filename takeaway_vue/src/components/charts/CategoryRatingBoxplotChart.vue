...
const loadData = async () => {
  try {
    const res = await merchantAPI.getCategoryRatingBoxplot()
    if (res.data.code === 200) {
      renderChart(res.data.data)
    }
  } catch (error) {
    console.error('加载品类评分箱线图失败:', error)
  }
}
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `${d.category}<br/>最小值: ${d.minRating}分<br/>下四分位: ${d.q1Rating}分<br/>中位数: ${d.medianRating}分<br/>上四分位: ${d.q3Rating}分<br/>最大值: ${d.maxRating}分<br/>平均值: ${d.meanRating}分<br/>店铺数: ${d.shopCount}家`
      },
    },
    xAxis: {
      name: '品类',
